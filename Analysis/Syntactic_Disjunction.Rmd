...
Generates a graph with each column being an item, error bars are +/- 2 SE, fill is by condition
```{r}
graph_data = left_join(means, key, by = 'ID') %>% mutate(ID = fct_reorder(ID, Mean_Chance))

g = ggplot(graph_data, aes(ID, Mean_Chance))
g = g + geom_col(position = "dodge", aes(fill = Condition)) + labs(title = 'Means and Variation in Inclusivity Ratings by Item', x = 'Items', y = 'Mean Inclusivity Rating (%)')+
  geom_errorbar(width = .5,
                aes(ymin = Mean_Chance - Standard_Error*2, ymax = Mean_Chance + Standard_Error*2),
                position = position_dodge(.9)) +
               theme_bw() +
               theme(axis.text.x = element_blank()) + labs(fill = "Condition")
...
```{r}
model_1 = lm(chance ~ Condition + Con_Length + Overall_Length + (1|workerid) + (1|Item), data = data)
summary(model_1)
```
